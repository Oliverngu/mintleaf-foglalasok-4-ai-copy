...
const usage = () => {
  ...
  console.log(
    '  tsx src/scripts/capacitySanityScan.ts --projectId=ID (or set PROJECT_ID / GCLOUD_PROJECT) [--limit=200]'
  );
  console.log('  tsx src/scripts/capacitySanityScan.ts --unitId=UNIT --from=YYYY-MM-DD --to=YYYY-MM-DD');
};
...
export const detectCapacityAnomalies = (rawDoc: unknown): AnomalyResult => {
  ...
  if (!hasTotalCount && !hasCount) {
    anomalies.push('missing-counts');
  }
  if (hasTotalCount && (totalCount === undefined || totalCount < 0)) {
    anomalies.push('totalCount-invalid');
  }
  ...
  if (Object.prototype.hasOwnProperty.call(rawDoc, 'byTimeSlot')) {
    ...
    if (!isRecord(byTimeSlotValue)) {
      anomalies.push('byTimeSlot-invalid');
    } else {
      let sum = 0;
      ...
      if (invalidSlot) {
        ...
        anomalies.push('byTimeSlot-sum-mismatch');
      }
      return {
        anomalies,
        totalCount,
...
const main = async () => {
  ...
  console.log(
    `[capacity-sanity] start projectId=${projectId ?? 'null'} projectIdSource=${projectIdSource} ` +
      `unitId=${unitId ?? 'all'} from=${from ?? 'null'} to=${to ?? 'null'} limit=${limit}`
  );
