...
export const computeReservationBucketKeys = ({
  ...
  // Buffer minutes are applied after endTime only (end boundary is exclusive).
...
export const applyCapacityLedgerTx = async ({
  ...
  for (const mutation of mutations) {
    ...
    const hasBucketDeltas =
      mutation.bucketDeltas && Object.keys(mutation.bucketDeltas).length > 0;
    if (mutation.totalDelta === 0 && !hasSlotDeltas && !hasBucketDeltas) continue;
