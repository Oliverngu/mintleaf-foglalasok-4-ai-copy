...
<div
className={w-8 h-8 rounded-full flex items-center justify-center font-bold transition-colors ${   isCompleted   ? 'bg-[var(--color-primary)] text-white'   : isActive   ? 'bg-green-200 text-[var(--color-primary)] border-2 border-[var(--color-primary)]'   : 'bg-gray-200 text-gray-500'   }}
>
{isCompleted ? '✓' : stepNumber}
</div>
<p
className={mt-2 text-sm font-semibold transition-colors ${   isActive || isCompleted   ? 'text-[var(--color-text-primary)]'   : 'text-gray-400'   }}
>
{label}
</p>
</div>
...
<div
className={flex-1 h-1 mx-2 transition-colors ${   isCompleted ? 'bg-[var(--color-primary)]' : 'bg-gray-200'   }}
></div>
)}
</React.Fragment>
);
  ...
  if (!adminRecipients || adminRecipients.length === 0) {  
    ...
    const detailsHtml = `  
      ...
      ">  
        ...
        <div style="margin-bottom: 12px;">  
          <strong>Auto-confirm:</strong> ${adminPayload.isAutoConfirm ? 'igen' : 'nem'}  
        </div>  
