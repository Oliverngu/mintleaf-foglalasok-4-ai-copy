    ...
    activeCombos
      ...
      .forEach(combo => {
        ...
        const zoneIds = new Set(
          comboTables.map(table => table.zoneId).filter(Boolean)
        );
        if (!zoneIds.size) {
          return;
        }
        ...
        const zoneId = Array.from(zoneIds)[0];
...
export const suggestAllocation = (
  ...
  const { partySize, bookingDate, seatingSettings, zones, override } = input;
  if (partySize <= 0) {
  ...
  const emergencyZoneIds = emergencyAllowed
    ? (seatingSettings.emergencyZones?.zoneIds ?? []).filter(zoneId =>
  ...
  if (allocationMode === 'floorplan' || allocationMode === 'hybrid') {
    ...
    if (emergencyZoneIds.length) {
      const emergencyResult = evaluateZones(emergencyZoneIds, 'EMERGENCY_ZONE');
      if (emergencyResult) {
